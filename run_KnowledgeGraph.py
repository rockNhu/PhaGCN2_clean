...
with open("name_list.csv",'w') as list_out:
    ...
    for file_n in file_list:
        for record in SeqIO.parse(contig_in+file_n, "fasta"):
            name = f"{str(old_file_id)}_{str(contig_id)}"
            contig_id += 1
...
db_fp = f"{args.database}/database.dmnd"
diamond_out_fn = (
    f"{os.path.basename(proteins_aa_fp).rsplit('.', 1)[0]}.diamond.tab"
)
...
_ = subprocess.check_call(f"cat {args.database}/database.self-diamond.tab.abc {contig_abc_fp} > {abc_fp}", shell=True)
...
description = ["hypothetical protein" for _ in protein_id]
...
_ = subprocess.check_call(f"cat {args.database}/ALL_gene_to_genomes.csv {out_f}contig_gene_to_genome.csv > {out_f}gene_to_genome.csv", shell=True)
...
if not os.path.exists(profiles_fp):
    ...
    print(f"File {profiles_fn} exists and will be used. Use -f to overwrite.")
...
# loading database
gene2genome = pd.read_csv(args.database+"/ALL_gene_to_genomes.csv")
...
name_to_id = {}
reference_df = pd.read_csv(args.database + "/reference_name_id.csv")
...
merged_df = pd.read_csv(args.database+"/ALL_genome_profile.csv", header=0, index_col=0)
Taxonomic_df = pd.read_csv(args.database+"/taxonomic_label.csv")
...
database_feature = pkl.load(open(args.database+"/dataset_compressF",'rb'))
